...
def test_placed_tile_scores_through_the_boards_starting_paths():
    board = Board.empty(4)
    place_tile(board, (1, 1), (1, 2), 2, 3)
    slots = [slot_index_of(4, 1, 0, Side.W), slot_index_of(4, 1, 3, Side.E)]

    assert score_lines(board, slots) == {frozenset(slots): 1}
    assert score_for(board, slots) == 1
